...
<div class="invoice-box">
    <h2>Edit Student Exam Mark Entry</h2>
    <p class="subtitle">Update student details and subject-wise marks</p>

    <form action="{{ url('studentexamresults/' . $studentExamResult->id) }}" method="POST">
        @csrf
        @method('PUT')
             @php
        ...
        <div class="invoice-details">
            <div class="field-group">
                ...
                <select name="student_name" id="student_name" required>
                    ...
                    @foreach ($students as $student)
                        <option value="{{ $student->id }}" {{ $studentExamResult->student_id == $student->id ? 'selected' : '' }}>
                            {{ $student->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="field-group">
                ...
                <input type="text" name="student_id" id="student_id" value="{{ $studentExamResult->student_id }}" required>
            </div>

            <div class="field-group">
                <label>Exam Type</label>
                <input type="hidden" name="student_exam_type_id" value="{{ $studentExamResult->student_exam_type_id }}">
                <p style="flex: 1; border: 1px solid #cecccc; padding: 4px 8px; font-size: 14px; font-weight: 600; color: #333; border-radius: 5px; height: 33px; line-height: 25px; display: flex; align-items: center;">
                    {{ $studentExamResult->examType->name }}
                </p>
            </div>

            <div class="field-group">
                <label>Exam Year</label>
                <input type="text" name="exam_year" value="{{ $studentExamResult->exam_year }}" required>
            </div>
        </div>
        ...
        <div class="invoice-details">
            <div class="field-group">
                ...
                <input type="number" step="0.01" name="total_marks" value="{{ $studentExamResult->total_marks }}" readonly>
            </div>
            <div class="field-group">
                <label>Average</label>
                <input type="number" step="0.01" name="average_marks" value="{{ $studentExamResult->average_marks }}" readonly>
            </div>
            <div class="field-group">
                <label>GPA</label>
                <input type="number" step="0.01" name="gpa" value="{{ $studentExamResult->gpa }}" readonly>
            </div>
            <div class="field-group">
                <label>Grade</label>
                <input type="text" name="grade" value="{{ $studentExamResult->grade }}" readonly>
            </div>
            <div class="field-group" style="flex: 1 1 100%;">
                <label>Remark</label>
                <input type="text" name="remark" value="{{ $studentExamResult->remark }}" style="width: 100%;" required>
            </div>
        </div>
        ...
        <table class="salary-breakdown">
            <thead>
                <tr>
                    <th>Subject Name</th>
                    @if($studentExamResult->student_exam_type_id == 1 || $studentExamResult->student_exam_type_id == 3)
                        <th>Marks</th>
                    @else
                        <th>Written</th>
                        <th>MCQ</th>
            ...
            <tbody>
                @foreach ($subjects as $index => $subject)
                    @php
                        $mark = $studentExamResult->markDetails->firstWhere('subject_id', $subject->id);
                    @endphp
                    <tr>
                        <td>
                        ...
                        @if($studentExamResult->student_exam_type_id == 1 || $studentExamResult->student_exam_type_id == 3)
                            <td><input type="number" name="subjects[{{ $index }}][written_marks]" value="{{ $mark->written_marks ?? '' }}" min="0" max="50" ></td>
                        @else
                            <td><input type="number" name="subjects[{{ $index }}][written_marks]" value="{{ $mark->written_marks ?? '' }}" min="0" max="70" ></td>
                            <td><input type="number" name="subjects[{{ $index }}][mcq_marks]" value="{{ $mark->mcq_marks ?? '' }}"min="0" max="30" ></td>
                            <td><input type="number" name="subjects[{{ $index }}][w_m_marks]" value="{{ $mark->w_m_marks ?? '' }}" readonly></td>
                        @endif
                        <td><input type="text" name="subjects[{{ $index }}][sgpa]" value="{{ $mark->gpa ?? '' }}" readonly></td>
                        <td><input type="text" name="subjects[{{ $index }}][sgrade]" value="{{ $mark->grade ?? '' }}" readonly></td>
                        <td><input type="text" name="subjects[{{ $index }}][remarks]" value="{{ $mark->remark ?? '' }}"></td>
                    </tr>
                @endforeach
            </tbody>
        </table>
...
<script>
  function calculateFinalTotals(form) {
    ...
    const avg = subjectCount ? totalMarks / subjectCount : 0;
    const avgGPA = subjectCount ? totalGPA / subjectCount : 0;
